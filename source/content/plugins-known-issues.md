...
**Issue 2:** Redirection prefers `$_SERVER['SERVER_NAME']` over `$_SERVER['HTTP_HOST']` for [URL and server](https://redirection.me/support/matching-redirects/) redirects. By default, `$_SERVER['SERVER_NAME']`` returns Pantheon's internal server name and not the current hostname, as a result, Redirection's `URL and server` based redirects never match.
